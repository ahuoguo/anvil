...
{
    zk.metadata().name().unwrap().concat("-configmap")
}
    ...
    ensures config_map@ == model_resource::make_config_map(zk@),
{
    ...
    config_map.set_data({
        ...
        data.insert("zoo.cfg".to_string(), make_zk_config(zk));
        data.insert("log4j.properties".to_string(), make_log4j_config());
        data.insert("log4j-quiet.properties".to_string(), make_log4j_quiet_config());
        data.insert("env.sh".to_string(), make_env_config(zk));
        data
    });
    ...
    ensures s@ == config_map::make_zk_config(zk@),
{
    ...
    metricsProvider.httpPort=".to_string().concat(i32_to_string(zk.spec().ports().metrics()).as_str()).concat("\n\
    metricsProvider.exportJvmInfo=true\n\
    initLimit=").concat(i32_to_string(zk.spec().conf().init_limit()).as_str()).concat("\n\
    syncLimit=").concat(i32_to_string(zk.spec().conf().sync_limit()).as_str()).concat("\n\
    tickTime=").concat(i32_to_string(zk.spec().conf().tick_time()).as_str()).concat("\n\
    globalOutstandingLimit=").concat(i32_to_string(zk.spec().conf().global_outstanding_limit()).as_str()).concat("\n\
    preAllocSize=").concat(i32_to_string(zk.spec().conf().pre_alloc_size()).as_str()).concat("\n\
    snapCount=").concat(i32_to_string(zk.spec().conf().snap_count()).as_str()).concat("\n\
    commitLogCount=").concat(i32_to_string(zk.spec().conf().commit_log_count()).as_str()).concat("\n\
    snapSizeLimitInKb=").concat(i32_to_string(zk.spec().conf().snap_size_limit_in_kb()).as_str()).concat("\n\
    maxCnxns=").concat(i32_to_string(zk.spec().conf().max_cnxns()).as_str()).concat("\n\
    maxClientCnxns=").concat(i32_to_string(zk.spec().conf().max_client_cnxns()).as_str()).concat("\n\
    minSessionTimeout=").concat(i32_to_string(zk.spec().conf().min_session_timeout()).as_str()).concat("\n\
    maxSessionTimeout=").concat(i32_to_string(zk.spec().conf().max_session_timeout()).as_str()).concat("\n\
    autopurge.snapRetainCount=").concat(i32_to_string(zk.spec().conf().auto_purge_snap_retain_count()).as_str()).concat("\n\
    autopurge.purgeInterval=").concat(i32_to_string(zk.spec().conf().auto_purge_purge_interval()).as_str()).concat("\n\
    quorumListenOnAllIPs=").concat(bool_to_string(zk.spec().conf().quorum_listen_on_all_ips()).as_str()).concat("\n\
    admin.serverPort=").concat(i32_to_string(zk.spec().ports().admin_server()).as_str()).concat("\n\
    ...
    ensures s@ == model_resource::make_env_config(zk@),
{
    ...
    "#!/usr/bin/env bash\n\n\
    DOMAIN=".to_string().concat(name.as_str()).concat("-headless.").concat(namespace.as_str()).concat(".svc.cluster.local\n\
    QUORUM_PORT=").concat(quorum_port.as_str()).concat("\n\
    LEADER_PORT=").concat(leader_election_port.as_str()).concat("\n\
    CLIENT_HOST=").concat(name.as_str()).concat("-client\n\
    CLIENT_PORT=").concat(client_port.as_str()).concat("\n\
    ADMIN_SERVER_HOST=").concat(name.as_str()).concat("-admin-server\n\
    ADMIN_SERVER_PORT=").concat(admin_port.as_str()).concat("\n\
    CLUSTER_NAME=").concat(name.as_str()).concat("\n")
}
