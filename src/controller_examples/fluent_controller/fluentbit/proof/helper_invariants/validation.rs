    ...
    ensures spec.entails(always(lift_state(daemon_set_in_etcd_satisfies_unchangeable(fb)))),
{
    ...
    let next = |s, s_prime| {
        ...
        &&& no_create_resource_request_msg_without_name_in_flight(SubResource::DaemonSet, fb)(s)
    };
    ...
    lemma_always_no_create_resource_request_msg_without_name_in_flight(spec, ds_res, fb);
    combine_spec_entails_always_n!(
        ...
        lift_state(no_create_resource_request_msg_without_name_in_flight(SubResource::DaemonSet, fb))
    );
...
//                assert(certain_fields_of_daemon_set_stay_unchanged(s_prime.resources()[ds_key], FluentBitView::unmarshal(s_prime.resources()[key]).get_Ok_0()));
                ...
                match step {
                    Step::ApiServerStep(input) => {
                        ...
                        if resource_create_request_msg_without_name(ds_key.kind, ds_key.namespace)(req) {} else {}
                    },
                    _ => {}
                }
            }
