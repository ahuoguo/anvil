...
pub open spec fn object_in_every_resource_create_or_update_request_msg_only_has_valid_owner_references(
    sub_resource: SubResource, fb: FluentBitConfigView
    ...
    ensures spec.entails(always(lift_state(object_in_every_resource_create_or_update_request_msg_only_has_valid_owner_references(sub_resource, fb)))),
{
    ...
    let next = |s, s_prime| {
        ...
        &&& FBCCluster::triggering_cr_has_lower_uid_than_uid_counter()(s)
        &&& FBCCluster::each_object_in_reconcile_has_consistent_key_and_valid_metadata()(s)
    };
    FBCCluster::lemma_always_triggering_cr_has_lower_uid_than_uid_counter(spec);
